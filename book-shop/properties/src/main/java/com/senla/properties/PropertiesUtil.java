...
public class PropertiesUtil {
    ...
    public static String getProperties(String filePah, String propertyName) throws PropertyNotFoundException {
        if (!propertiesFilesMap.containsKey(filePah)){
            ...
            try (FileInputStream file = new FileInputStream(FOLDER + filePah)){
                properties.load(file);
            }catch (IOException e){
                LOGGER.error(EXCEPTION_PROPERTIES_FILE_LOAD + System.getProperty("user.dir") + "\\" + filePah);
        ...
        if (properties == null){
            ...
            LOGGER.error(new Formatter().format(EXCEPTION_PROPERTIES_LOAD_FMT, System.getProperty("user.dir") + "\\" + filePah, propertyName).toString());
